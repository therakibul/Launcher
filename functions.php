...
function launcher_assets() {
    ...
    wp_enqueue_style( "launcher", get_stylesheet_uri(),null, time());
    ...
    if(is_page() && is_page_template("launcher.php")){
        $year = get_post_meta(get_the_ID(), "year", true);
        $month = get_post_meta(get_the_ID(), "month", true);
        $day = get_post_meta(get_the_ID(), "day", true);

        wp_localize_script("main-jquery-js", "dateInfo", ["year" => $year, "month" => $month, "day" => $day]);
    }
}
